...
export const supabaseService = {
  ...
  async getKPIs(filtros?: Filtros): Promise<KPIData> {
    try {
      ...
      console.log('  - Pendientes:', eventosPendientes);
      console.log('  - Rechazados:', eventosRechazados);
      ...
      // Calcular comisiones basadas en los eventos completados filtrados (8% del precio final)
      const comisionesTotales = eventos
        ?.filter((e: Evento) => e.estado === 'completado' && e.precio_final)
        .reduce((sum: number, e: Evento) => sum + ((Number(e.precio_final) || 0) * 0.08), 0) || 0;

      // Extraer DJs únicos de los eventos filtrados
      const djsEnEventos = new Set(eventos?.map(e => e.dj_id).filter(Boolean));
      const djsActivosFiltrados = djs?.filter((dj: { id: string; activo: boolean }) => 
        dj.activo && djsEnEventos.has(dj.id)
      ).length || 0;

      // Extraer Clientes únicos de los eventos filtrados
      const clientesEnEventos = new Set(eventos?.map(e => e.cliente_id).filter(Boolean));
      const clientesActivosFiltrados = clientes?.filter((c: { id: string; activo: boolean }) => 
        c.activo && clientesEnEventos.has(c.id)
      ).length || 0;

      // Calcular rating promedio solo de DJs filtrados
      const djsParaRating = djs?.filter((dj: { id: string; activo: boolean }) => 
        djsEnEventos.has(dj.id)
      ) || [];
      
      const ratingPromedioDJs = djsParaRating.length > 0
        ? djsParaRating.reduce((sum: number, dj: { rating: number }) => sum + (Number(dj.rating) || 0), 0) / djsParaRating.length
        : 0;
      ...
      const kpiData: KPIData = {
        ...
        eventosRechazados,
        eventosPendientes,
        djsActivos: djsActivosFiltrados,
        clientesActivos: clientesActivosFiltrados,
        ingresosTotales,
        comisionesTotales,
        ratingPromedioDJs,
        tasaConversion,
  ...
  async getEventosPorTipo(filtros?: Filtros): Promise<ChartData[]> {
    try {
      ...
      eventos?.forEach((evento: { tipo_evento: string }) => {
        // Filtrar eventos con tipo_evento válido (no nulo, no vacío, no espacios)
        if (evento.tipo_evento && evento.tipo_evento.trim() !== '') {
          const tipoLimpio = evento.tipo_evento.trim();
          eventosPorTipo[tipoLimpio] = (eventosPorTipo[tipoLimpio] || 0) + 1;
        }
      });

      const resultado = Object.entries(eventosPorTipo)
        .filter(([tipo]) => tipo && tipo.trim() !== '') // Doble filtro por seguridad
        .map(([tipo, cantidad]) => ({
          fecha: tipo, // se reutiliza propiedad 'fecha' como etiqueta categórica
          valor: cantidad
        }))
        .sort((a, b) => b.valor - a.valor); // Ordenar por cantidad descendente

      console.log('📊 Distribución por tipo (filtrado):', resultado);
      console.log('📊 Tipos válidos encontrados:', resultado.length);
      return resultado;
    } catch (error) {
